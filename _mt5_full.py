...
for name, login, password, server in accounts:
    ...
    if deals:
        ...
        print(f'  Total deals in MT5 history: {len(real)} buy/sell trades')
        closed = [d for d in real if d.entry == 1]
        last3 = sorted(closed, key=lambda d: d.time)[-3:]
        print(f'  Last 3 closed trades:')
        for d in last3:
            t = datetime.fromtimestamp(d.time).strftime('%Y-%m-%d %H:%M')
            side = 'BUY' if d.type == 0 else 'SELL'
            print(f'    {t} | {d.symbol:10s} {side} {d.volume}L | exit={d.price:.5f} | profit=${d.profit:.2f} | comment={d.comment}')
        ...
        if positions:
            ...
            for p in positions:
                ...
                print(f'    {p.symbol} {side} {p.volume}L | entry={p.price_open:.5f} | P&L=${p.profit:.2f}')
        else:
            print(f'  Open positions: none')
    else:
        print(f'  No MT5 history: {mt5.last_error()}')
        positions = mt5.positions_get()
        if positions:
            print(f'  Open positions ({len(positions)}):')
            for p in positions:
